...
def main(ymd, cc):
    src = f"gfs_cache/v17replay/scenario_3comp_24h_ai_{ymd}{cc}.npz"
    d = np.load(src)
    ...
    fig.text(0.01, 0.004,
             ...
             "Method: Quantile Delta Mapping (Cannon et al. 2015) applied to NOAA AIGFS v1.0 (GraphCast) cycle "
             ...
             "Boundaries: Natural Earth + GADM v4.1  |  Display grid ~3 km (interpolated from 0.25\u00b0 GFS)  |  AIGFS provides total precip only (ACPCP is placeholder-zero, verified) — convective share from v17 reference climatology  |  Experimental guidance only",
             fontsize=6.8, color='#555555', ha='left', va='bottom')

    output = f"hadramout_ai_total_24h_{ymd}_{cc}Z.png"
