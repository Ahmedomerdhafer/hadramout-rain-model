...
===============================================================================
  ...
  المنخفضة (الطبقية غير الحملية) — الأيام العشرة القادمة — محافظة حضرموت
  نمذجة سيناريوهية وفق دوال النظام التجريبي NOAA GFS v17-HR1
===============================================================================
  الدورة         : 2026-09-14 18Z (أحدث دورة مكتملة عند الإنتاج)
                   النافذة: 14 سبتمبر 18Z → 24 سبتمبر 18Z
...
OUTPUT = "hadramout_ai_3comp_20260924_00Z_panel.png"
...
def _panel(ax, field, lats, lons, arabic_fonts, title_ar, title_en):
    ...
    return cf   # (أُزيلت أسماء المديريات بطلب المستخدم — تبقى في خرائط 24 ساعة)


def main():
    d = np.load("gfs_cache/v17replay/scenario_3comp_ai_2026092400.npz")
    ...
    axes[0].text(0.015, 0.985,
                 "Window: 24 Sep 2026 00Z → 04 Oct 2026 00Z (10 days)\n"
                 "Cycle: 20260924/00Z — AI-GFS (GraphCast), complete run",
    ...
    p1, p3 = axes[0].get_position(), axes[2].get_position()
    cx = (p1.x0 + p3.x1) / 2.0
    fig.suptitle(_ar("محافظة حضرموت — إجمالي الأمطار والأمطار الرعدية وأمطار السحب المنخفضة خلال الأيام العشرة القادمة\n"
    ...
    fig.text(0.01, 0.003,
             ...
             "Method: Quantile Delta Mapping (Cannon et al. 2015) applied to NOAA AIGFS v1.0 (GraphCast) cycle 20260924/00Z "
    ...
    # ===== تقرير المحطات =====
    print(f"\n📊 النقاط المرجعية (دورة 24/00Z — AI-GFS):")
    print("-" * 88)
